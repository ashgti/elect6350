...
class Costmap2D(object):
    ...
    The dtype must match an option for dtype on a numpy array, default is
      'int16'.
    The resolution parameter can be thought of as the ration of units to cells.
      For example, with a costmap of 10x20 units and a resolution of 0.5, the
      actual grid would consist of 20x40 cells.
    """
    def __init__(self, width=1, height=1, dtype='int16', resolution=1.0):
        ...
        if self.width <= 0 or self.height <= 0:
            raise ValueError("Invalid width or height ({}x{}), less than or \
                              equal to zero."\
